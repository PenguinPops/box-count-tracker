...
function ReportContent({
    ...
    const entries = [...originalEntries].sort((a, b) => {
        // Convert YYYYMMDD to number for comparison
        const dateA = typeof a.entry_date === 'string' && /^\d{8}$/.test(a.entry_date)
            ? parseInt(a.entry_date, 10)
            : new Date(a.entry_date).getTime();
            
        const dateB = typeof b.entry_date === 'string' && /^\d{8}$/.test(b.entry_date)
            ? parseInt(b.entry_date, 10)
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="mt-8 pt-4 border-t text-sm text-gray-500">
                <p>
                    {t(language, 'generatedOn')} {new Date().toLocaleDateString()} {t(language, 'at')} {new Date().toLocaleTimeString()}
                </p>
            </div>
        </div>
    )
